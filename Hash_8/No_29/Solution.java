...
나중에 해시 방법으로 풀어봐라
라빈 카프 알고리즘은 충돌의 가능성이 있기 때문에 해시 값이 맞을 때 문자열을 검사하는 과정이 필요는 하지만, PS에서는 그런 과정을 다 하기에는 시간 초과가 날 수 있습니다. 킹고리즘님이 적었뜻이, 선형탐색으로 체크하는 코드를 빼시면 시간 초과 자체는 나지 않을 겁니다.

하지만, 그럼 충돌이 났을 때는 어떻게 해야할까요?
결국에 라빈 카프는 데이터 개수가 많아지면 충돌이 나게 할 수 있습니다. 하지만, PS에선 이를 피하기 위해서 이중 해시, 삼중 해시 등을 사용합니다. 따라서 라빈 카프에 사용하는 수들을 좀 더 잡은 뒤에 1번 라빈 카프랑 비교하고, 둘의 해시 값이 같으면, 2번 라빈 카프랑 비교해서 둘 다 해시 값이 같으면 같은 문자열이라고 판단하고 넘어가는 이중 해시를 사용하면 PS에선 사실상 충돌을 다 피할 수 있습니다.
