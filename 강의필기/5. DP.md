...
1. Brute-force 접근 방법(모든 부분 집합 찾아서 계산, O(2^n))
2. DP 접근 방법(O(n))
...
<img src="https://user-images.githubusercontent.com/107410759/216234135-566bec69-acfa-43aa-b481-c05a0409c7ef.png" width="600" height="200"/>


### 문제 제시: 모든 쌍 최단 경로
<img src="https://user-images.githubusercontent.com/107410759/216239263-58365ad9-69a5-42e3-8f21-97f54a08b1c1.png" width="800" height="400"/>
- 중요 아이디어: 경유하는 지점을 1개씩 늘리면서 최단 거리를 계산하며 결과를 갱신한다.
- 부분문제 정의
    + 입력 그래프의 정점을 각각 1,2,3,...,n이라 하자.
    + Dijk = 점 {1,2,...,k}만을 경유 가능한 점들로 고려하여, 점 i로부터 점 j까지의 모든 경로 중에서 가장 짧은 경로의 거리
    + 점 k를 경유하는 경로의 거리는 Dik(k-1) + Dkj(k-1)
- 주의할 점: 경유 가능한 모든 정점들을 반드시 경유해하는 경로를 의미하는 것이 아니다.

1. Brute-force 접근 방법: 모든 경로의 길이를 구한 뒤, 그들 중에서 최소길이를 찾는다. -> 모든 경로의 개수: (n-2)!
2. DP 접근 방법
    - 다익스트라 알고리즘: O(n^3)
    - 플로이드 알고리즘: O(n^3)
...
### 문제 제시: 최소여행경비
- 휴가 기간에 다음 도시들을 최소 이동 경비로 여행하고 싶다. 어느 순서로 여행하면 되나? 단 집에서 출발해서 여행이 끝나면 집으로 돌아와야 한다.
<img src="https://user-images.githubusercontent.com/107410759/216245763-4edb8aee-205b-4669-ac0f-f39cd139126a.png" width="300" height="300"/>

1. Brute-force 접근 방법: 모든 여행 경로를 고려(가능한 일주 여행 경로의 총 개수는 (n-1)!이다.)
2. DP 기반 접근 방법
    - V는 모든 정점의 집합, A는 V의 부분집합이라고 하자
    - D[vi][A]는 A에 속한 각 정점을 정확히 한번씩만 거쳐서 vi에서 v1으로 가는 최단 경로의 길이라고 하자.
    - ex. D[v2][] = 1, D[v4][] = 6, D[v4][{v2}] = 3+1 = 4

    - 시간 복잡도: O(n^2 * 2^n)
    - 공간 복잡도: O(n*2^n)

<img src="https://user-images.githubusercontent.com/107410759/216247143-344a302c-fe12-4d0a-8490-cc50e1e081ed.png" width="800" height="600"/>
<img src="https://user-images.githubusercontent.com/107410759/216248110-49dba9ee-da68-466b-868d-6a50b5f0e83e.png" width="400" height="100"/>

```
const number W[][], number & minlength
...
travel(n, W[][], minlength)
    ...
    for k in 1 -> n-2
        for all subsets of V-{v1}(A라 칭함) containing k vertices
            for i such that i != 1 and vi 가 A에 속하지 않을 때
                D[i][A] <- mininum(A에 속하는 vj)(W[i][j] + D[vj][A-{V1}])
...
int dp(int here, int visited) {
    ...
    ret = INF;
    if (visited == (1 << N) - 1) //모든 마을을 방문했을 떄
...
### TSP - Bitmask를 이용한 집합표현
- 이미 방문했던 도시들을 재 방문하면 안되기 때문에. 방문했던 도시들을 체크하는 방법으로 bitmask를 사용한다.

### TSP - Brute Force vs DP
<img src="https://user-images.githubusercontent.com/107410759/216253825-2d0236e5-d686-4443-9c8d-43a84f4f2eea.png" width="800" height="600"/>
